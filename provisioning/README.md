...
| `dataverse/` | `verify-solution-components.ps1` | Read-only live-environment counterpart to `scripts/verify-solution-root-components.py`: EVERY declared component type — table (+ attributes, IMP-0122), global option set, relationship, security role (+ privileges), cloud flow, sitemap, field security profile (+ permissions), model-driven app, environment variable definition, plus systemform/savedquery (never their own RootComponent — they ride in as entity subcomponents) — checked live, with the list derived from `--emit-json`, never hand-typed (IMP-0013: a hand-typed list once silently omitted systemform/savedquery, and that is exactly how the first DEV deploy was called "verified"). Optionally re-runs a solution import once to prove idempotency (`-SolutionZipPath`). Reads `dev-solution-verification-settings.json` for DEV, since `Get-ProvisioningSettings -Env dev` throws by design → `PASS`/`FAIL` | — |
