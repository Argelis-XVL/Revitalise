...
|---|---|---|---|---|---|
| C-DOM-001 | Personal data must be classified before any entity is designed or implemented, and the classification must match the access control the solution actually implements | HARD | plan-agent, architect-agent | GDPR Art. 30 | `python3 scripts/verify-design-doc-claims.py docs/architecture docs/plans` — the `design-doc-claims` build step. **Amended 2026-08-28 (`IMP-0374`, `IMP-0376`, `IMP-0379`, `IMP-0391`):** the previous `Verify By` was *"TAD §3 data classification column is fully populated"*, a COMPLETENESS test that a populated-but-wrong row passes by construction — and one did, for the whole life of two approved documents. Populated is still necessary and is no longer sufficient: the checkable claims are now compared against source (an existing `rev_*` column asserted absent; every stated contrast ratio recomputed from the hex values its own row names) |
...
| C-DOM-033 | **Every column-secured attribute in this solution is adjudicated in the special-category register — in `columns:` or in `pending_adjudication:`, never neither and never both.** A decision to apply column-level security is a decision that the data is sensitive; this row requires that the *reason* be written down where the register's readers will find it. `columns:` means adjudicated AS Article 9 / highest-tier, with a basis. `pending_adjudication:` means secured, with the Article 9 call not yet made by the party who owns it. **What this row does NOT require is that the classification be correct** — Article 9 membership is a legal judgement about what data means, and the owner of that judgement is the Domain Owner / Compliance Lead, not a script | HARD | development-agent, data-agent, architect-agent, build-agent | `IMP-0598` (**blocker**). `constraints/domain/special-category-register.yml` opens by declaring itself *"the single source of truth for which columns of this solution hold UK GDPR Article 9 special-category data"*, and did not list `rev_applicant.rev_ethnicgroup` — a column live since **2026-08-27** whose own `Entity.xml` `<Description>` reads *"UK GDPR ARTICLE 9 special-category data (Tier 4)"*. It was correctly secured (`IsSecured=1`, `IsAuditEnabled=1`, in `REV_TrusteeRestricted`) and correctly barred from scoring; it was simply never registered, for **eight days**, and was found incidentally while fixing an unrelated form-control defect. **`verify-domain-invariants.py` passed 20/20 throughout, and that is the generalisable point:** all four of C-DOM-030/031/032/004 read the register and ask whether SOURCE agrees with IT. Not one asked the converse — whether the register is COMPLETE against source — so the one defect none of them can see is the omitted row. The register's own `HOW TO ADD A COLUMN` procedure already listed every required step; the session that built the column was scoped to schema and flow work and never opened the file. **A procedure that depends on being remembered is the failure mode `C-TECH-060` was created to end**, and this is its fourth instance against a hand-kept list in this codebase after `IMP-0038` (the `ensure-schema.ps1` entity list), `IMP-0155` and `IMP-0212` (absolute counts in the Pester suites). Class `declared-policy-not-mechanically-enforced`, ×27 | `python3 scripts/verify-domain-invariants.py src/solutions/RevitaliseGrantAutomation --register constraints/domain/special-category-register.yml --build-config config/revitalise-grant-automation-build.yml` (build step `domain-invariants`, HARD) exits 0 — *grep that `steps:` block to check this row, never a review's APPLIED cell (`IMP-0174`)*. **No new script and no new build step**: the check went into the gate that already owns this register, so the `57` `verify-*.py` count in `scripts/derived-counts-registry.json` is unchanged. **It asserts on VALUES against an ENUMERATED corpus, fail-closed** — the `IsSecured` element on one side, two declared lists on the other. **The finding's own proposed design was measured and REJECTED**, and the numbers are why: over all 226 attributes on 2026-09-04, scanning `<Description>` text for Article-9 language gives **9 findings / 1 true positive (11%)**, and its false positives are the columns whose authors documented NON-membership most carefully — `rev_gender`'s *"gender is not a UK GDPR Article 9 category"*, `rev_hearaboutus`, `rev_review.rev_notes1`, and the five `*redacted` counterparts that cite their Article 9 SOURCE. Better documentation, **more** findings: the inverted polarity `IMP-0422`/`IMP-0428` record, measured here for the sixth time. `IsSecured` alone gives **52 / 1 (2%)**; `REV_TrusteeRestricted` membership **36 / 1 (3%)**, and that one fails for a *stated* reason — the profile's own `description` releases *"the Tier 4 identifying AND special-category columns"*, so it was never built to separate them. Per `IMP-0560`, the corpus was enumerated **before** the set was chosen: **68 secured = 17 registered + 51 pending, 0 undeclared**, so day-one findings are **0 and 0 is correct**. **Proven able to fail, four ways**, not merely selftested: `rev_ethnicgroup` removed from both lists → exit 1 naming it (this is `IMP-0598` itself, reproduced); a column in both lists → exit 1; a `pending_adjudication` row for a non-existent column → exit 1; and a **new** secured column (`rev_religiousbelief` fixture) declared nowhere → exit 1 naming it. The pending count prints on the PASS path too, because a figure computed and discarded on the green branch is `IMP-0511`. RESIDUALS, stated because they are not covered: this row cannot tell a correct adjudication from a wrong one, and **`pending_adjudication:` is a DEBT, not a clearance** — 51 columns are enumerated there today and none of them has been adjudicated by their owner; and an Article 9 column that is **not** column-secured is invisible here, which is why C-DOM-031's `secured: exception` route keeps its own reason-and-owner requirement |
