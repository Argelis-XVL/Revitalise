...
| C-COM-006 | No client-facing document claims a verification level above the level actually reached. **V6 (client accepted) is the EARLIEST of the three routes Build Terms B5 opens** — written confirmation, ten business days' silence after the phase is submitted for acceptance, or a deliverable being put into live operational use — and each route's date comes from a human, never from an agent | HARD | acceptance-agent, commercial-agent | `C-TECH-053` applied to a contract. `IMP-0012`: three components imported cleanly, were queryable, and no maker could open them. **Amended 2026-08-19 (`IMP-0072`): the original row said V6 could only be recorded from an explicit `CLIENT ACCEPTED` input. B5 says otherwise, and two of its three routes need nobody to do anything — so the original row would have missed an acceptance that had already happened.** Warranty, the liability cap and the support boundary all hang off the date | `scripts/verify-acceptance-pack.py`, `scripts/warranty-clock.py` (reads all three routes, reports the earliest) |
...
| C-COM-011 | Every authorising commercial act records, **before the act**, the human who authorised it (`authorised_by`, a named person — never "reviewer") and the channel the keyword arrived through (`relayed_by`: `lead-agent` or `direct`). An entry missing either is not an authorisation | HARD | commercial-agent, pm-agent, acceptance-agent | `IMP-0787`: a relayed `APPROVE CHANGE ORDER CO-006` was refused three times, two days after the same role accepted the same channel three times and wrote it into the ledger each time — because no file said which was right. Nothing in this harness authenticates a human, so the record is the whole control: a named authoriser is what lets the reviewer repudiate an authorisation they did not give, while the act is still revertible. Rule: `agents/WORKFLOW.md` → *What channel a keyword must arrive through* | `scripts/verify-commercial-events.py` check 3 + its `an-authorising-entry-naming-no-human-fails` and `an-authorising-entry-with-no-channel-fails` fixtures |
