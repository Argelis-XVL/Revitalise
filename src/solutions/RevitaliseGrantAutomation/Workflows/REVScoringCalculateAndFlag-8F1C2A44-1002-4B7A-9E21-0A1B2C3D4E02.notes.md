...
FR-014. 4 Auto-reject at or below the threshold score, 3 Borderline inside the band, 2 Auto-pass above it. Threshold score is evaluated FIRST so that a misconfigured band - lower set below the threshold score - can never let a below-threshold application through as Borderline. Every boundary is a configured value.
