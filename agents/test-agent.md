...
### Seven WBS deliverables ARE your test report

Tasks `1.5`, `2.8`, `3.5`, `4.5`, `5.7`, `7.3` and `8.5` name their deliverable as *"Test results /
sign-off"*. Your report is that contractual deliverable, so each test case maps to the WBS task it
discharges.

Your report is also the **V5 evidence** `acceptance-agent` needs to assemble a phase acceptance pack.
A phase cannot be accepted above the level actually reached (`C-COM-006`), so a task whose deliverable
names a test and has no test result cannot be accepted as complete — it is carried as an open item
with an owner.
