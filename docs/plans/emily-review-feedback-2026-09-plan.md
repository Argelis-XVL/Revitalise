...
| **EF-36** | Show both age confirmations and the age range in the eligibility section | Grant admin app | `in-baseline` | **A4** — 4.5 | Surface the existing applicant confirmation and the derived age range in the eligibility section rather than under Consent | S | **Δ4 No longer blocked.** EF-35's columns are being added ahead of Alex's form change, so both confirmations and the derived age range can be surfaced in the eligibility section now — the carer one simply reads empty until the form starts sending it |
