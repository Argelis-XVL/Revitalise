...
**Correction (this fix, 2026-09-18): the `A-FIN-03` row this table used to carry here — the Decimal
control classid on `rev_roundfinance`'s form — is removed as a stale duplicate, not merged.** That id
was already allocated, and closed, to an unrelated claim (`REV_FinanceOnly`'s `fieldsecurityprofileid`)
in `docs/development/revitalise-grant-automation-dev-summary.md` (line 5409, VERIFIED 2026-08-23). This
document's row restated the same collision `IMP-0703` fixed once already: that fix (2026-09-10) corrected
the orphaned source marker on `rev_roundfinance`'s form header from `A-FIN-03` to a fresh `A-FIN-08` and
added `A-FIN-08`'s own register row in `revitalise-grant-automation-dev-summary.md` — but did not know
this document also carried an independent copy of the same claim under the same colliding id, so the
duplicate survived the fix that supposedly closed it. The claim itself is unchanged and tracked in exactly
one place now: `A-FIN-08` (OPEN), `revitalise-grant-automation-dev-summary.md` §10, line 7642. Every
reference below that used to read `A-FIN-03` for this claim now reads `A-FIN-08`.
...
whether the rendered result looks the way the arithmetic says it does, and A-DS-12 is the narrowest true
statement of that.
...
- **`A-FIN-08`** (Decimal control classid on the `rev_roundfinance` form — renumbered from `A-FIN-03` by this
  fix, 2026-09-18; see the correction note above §10): the live `systemform` row (queried
...
- **`A-DS-12`** (muted/quiet visual distinction, sighted-user judgement): no mechanism exists to verify this
...
with no recorded session (`A-FIN-08`, `A-DS-12`) — and `A-TR-13` names data that does not exist yet either way.**
...
residual every prior revision of this screen carries (A-DS-12, §10), unmoved by a source-level revision.
...
of this screen already carries (A-DS-12, §10 — no live signed-in-trustee V4 session) is unmoved. It also
...
- [ ] **A-DS-12 is OPEN and is the one thing a reviewer should look at rather than read about** — whether the
...
- [x] Every guessed platform contract is in §10 and commented in source — **this document now carries 12
      rows** (`A-FLOW-01..06`, `A-LAND-1..4`, `A-FIN-08`, `A-TR-13`); 10 OPEN, 2 CLOSED (`A-LAND-1`,
